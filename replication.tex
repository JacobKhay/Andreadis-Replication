...
\section{Reproduction of Original
...
though all other variables replicate precisely. The spatial maps also
reveal inconsistencies with the published figure: Connecticut does not
appear in the authors' data.csv file (coded as FIPS 9999 for state-level
rather than individual county codes), resulting in N/A values in our
replication. Additional visual discrepancies exist between our
replicated maps and the published maps, which could reflect a copy-paste
error when the authors merged data files or use of a different data
source not included in the shared replication materials.
...
\subsection{Interpretation of Table 1
Results}\label{interpretation-of-table-1-results}
...
\subsection{Interpretation of Table 2
Results}\label{interpretation-of-table-2-results}
...
California (6.34 percent), with an overall average of 0.45 percent and
median of 0.23 percent across all counties. Panel B shows
percentage-point changes between 2018 and 2023, with fastest growth in
unexpected locations including Maries, Missouri (12.35 percentage
points), Hughes, South Dakota (10.43 percentage points), and Osage,
Michigan (9.82 percentage points), reflecting shifts toward suburban and
remote-friendly regions. High AI adoption counties tend to be coastal
metropolitan areas and established tech ecosystems, while rural inland
regions show minimal activity. Counties with initially high AI shares
continued strengthening their position, while most experienced modest
growth, suggesting persistent geographic concentration in AI job
markets. Connecticut appears with N/A values because the authors'
data.csv file coded Connecticut data with FIPS 9999 (state-level) rather
than individual county codes. Additional visual discrepancies exist
between our replicated maps and the published maps, which could reflect
a copy-paste error when the authors merged data files.}

\end{figure}%

\newpage

\subsection{Interpretation of Figure 1
Results}\label{interpretation-of-figure-1-results}
