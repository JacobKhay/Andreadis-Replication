...
author:
 - Jacob Khaykin\textsuperscript{*}
 - David Kane\textsuperscript{†}
 ...
 pdf:
   ...
   include-in-header:
     - text: |
         ...
         \newcommand{\italicblock}[1]{\textit{#1}}
...
\textsl{JEL: J24, O33, R11}
\textsl{Keywords: Artificial Intelligence, Regional Economics, Labor Markets}
\textsl{Data Availability: The R code and data to reproduce this replication are available in this repository: https://github.com/JacobKhay/JustReplicationAI-Share.}

<!-- JK: Shoiuld repo be linked? -->
...
\textsuperscript{*}Solon High School, jacobkhaykin27@solonschools.net  
\textsuperscript{†}Institute for Globally Distributed Open Research and Education (IGDORE), dave.kane@gmail.com  
...
# Quietly load county geometries (cartographic boundaries = lighter/faster)
# Filter out Puerto Rico (FIPS codes starting with 72)
...
# Apply shift_geometry to include AK and HI as shifted insets
map_data <- tigris::shift_geometry(map_data)
...
# Enhanced map theme with better sizing
map_theme <- theme_void() +
  theme(
    ...
    legend.key.size = unit(2, "cm"),   # <--- KEY SIZE MADE BIGGER
    plot.margin  = unit(c(0.5, 0.5, 0.5, 0.5), "cm"),
    aspect.ratio = NULL
  )

# Plot for average with fixed coordinates for shifted geometry
p1 <- ggplot(map_data) +
  ...
  labs(
    title   = "Panel A. Percent share of AI jobs, 2014–2023",
    caption = "Panel A plots the share (average 2014–2023) of job postings related to AI in a county."
  ) +
  map_theme

# Plot for change with fixed coordinates for shifted geometry
p2 <- ggplot(map_data) +
  ...
  scale_fill_manual(values = cols_chg, na.value = 'gray90', drop = FALSE, 
                    guide = guide_legend(title = "", override.aes = list(size = 0))) +
  labs(
    title   = "Panel B. Percentage point change in AI share, 2018–2023",
    caption = "Panel B plots the percentage point change (2023–2018) in the share of AI jobs."
  ) +
  map_theme

# Create final plot with better spacing and sizing
final_plot <- (p1 / p2) +
  plot_annotation(
    title = "Figure 1: Replication of Figure 1 from Andreadis et al. (2025) - Spatial Heterogeneity in the Share of and Change in AI Jobs",
    theme = theme(
...
The original study by @andreadis2025 employs language that suggests causal relationships despite relying on observational data with multiple potentially endogenous predictors. Several passages from the paper illustrate this concern:
...
wrap_plots(plots, ncol = 2) +
  plot_annotation(
    title = "Figure 2: Table 1 — Model-by-Model Panel Plot (Original vs Log-Pop Weighting)",
...
wrap_plots(plots, ncol = 2) +
  plot_annotation(
    title = "Figure 3: Table 2 — Change in AI Share (Original vs Log-Pop Weighting)",
    theme = theme(plot.title = element_text(size = 14, face = "bold", hjust = 0.5))
  ) &
  plot_layout(guides = "collect") &
...
We acknowledge @andreadis2025 for making their methodology transparent, enabling this replication study. All errors remain our own.
