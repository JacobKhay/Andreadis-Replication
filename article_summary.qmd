  ...
  pdf:
    ...
    include-in-header:
      - text: |
          ...
          \usepackage{fancyhdr}
          \pagestyle{fancy}
          \fancyhead[C]{\small Journal of Comments and Replications in Economics - JCRE}
          \fancyfoot{}
          \renewcommand{\headrulewidth}{0.4pt}
          \usepackage[dvipsnames]{xcolor}
          \pretitle{\begin{center}\LARGE\color{MidnightBlue}}
          \posttitle{\par\end{center}\vskip 0em}
          \setlength{\headheight}{15pt}
          \setlength{\headsep}{20pt}
          \setlength{\droptitle}{5em}
          \usepackage{sectsty}
          \allsectionsfont{\bfseries\color{black}}
          \usepackage{parskip}
...
JEL: J24, O33, R11  
Keywords: Artificial Intelligence, Regional Economics, Labor Markets, Replication, Robustness  
Data Availability: The R code and data to reproduce this replication are available in JCRE's data archive (DOI: xxxxx).

Please Cite As: Khaykin, J. and Kane, D. (2025). Local Heterogeneity in Artificial Intelligence Jobs Over Time and Space: A Replication Study of Andreadis et al. (AEA Papers and Proceedings, 2025). Journal of Comments and Replications in Economics, Vol.x (202x-x). DOI: xxxxx

## Abstract

This paper replicates the analysis of [@andreadis2025] on the local heterogeneity in artificial intelligence jobs across U.S. counties from 2014 to 2023. [@andreadis2025] document substantial variation in AI-related job postings and identify key correlates including education, innovation, and industry factors. We successfully reproduce their main results and extend the analysis by employing log-population weights to assess robustness. The core associations persist, though some magnitudes attenuate. Additionally, we highlight unsupported causal claims in the original study given its observational design. Our findings affirm stable correlational patterns while emphasizing caution in interpretation. Replication code and data are provided.
\end{abstract}

$^{*}$Solon High School, jacobkhaykin@example.com  
$^{†}$Institute for Globally Distributed Open Research and Education (IGDORE), davidkane@example.com  
Declaration: This paper was not the result of a for-pay consulting relationship. Neither we nor our employers have a financial interest in the topic that could create a conflict of interest.

Received xxxxx; Revised xxxxx; Accepted xxxxx; Published xxxxx.  
...
The original study by [@andreadis2025] employs language that suggests causal relationships despite relying on observational data with multiple potentially endogenous predictors. Several passages illustrate this concern:
...
The regression specifications employed by [@andreadis2025], while appropriate for documenting correlational patterns, face significant challenges for causal interpretation:
...
I thank my preceptor for guidance on this replication exercise and extension analysis. I also acknowledge [@andreadis2025] for making their methodology transparent, enabling this replication study. All errors remain my own.
