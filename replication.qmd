...
<!-- JK: realized that the panel plot font(s) might be too small. Sorry, will fix next time.  -->
...
data_ai_z <- data_ai |> 
  filter(emp != 0) |>                               
  mutate(
    share_bac = scale(share_bac)[,1],                   
    share_black = scale(share_black)[,1],               
    share_poverty = scale(share_poverty)[,1],           
    logpop = scale(logpop)[,1],                         
    hpi_ch = scale(hpi_ch)[,1],                         
    logincome = scale(logincome)[,1],                   
    tightness = scale(tightness)[,1],                   
    unrate = scale(unrate)[,1],                         
    pat_intensity = scale(pat_intensity)[,1],           
    patai_intensity = scale(patai_intensity)[,1],       
    degshare = scale(degshare)[,1],                     
    stemshare = scale(stemshare)[,1],                   
    large_firms = scale(large_firms)[,1],               
    information_intensity = scale(information_intensity)[,1],  
    manuf_intensity = scale(manuf_intensity)[,1],       
    TurnOvrS = scale(TurnOvrS)[,1],                     
...
obs <- map_chr(models, ~comma(nobs(.x)))
r2_vals <- map_chr(models, ~sprintf("%.3f", r2(.x, "r2")))
wr2_vals <- map_chr(models, ~sprintf("%.3f", r2(.x, "wr2")))
...
extra_rows <- tribble(
  ...
  "Within R²", wr2_vals[1], wr2_vals[2], wr2_vals[3], wr2_vals[4], wr2_vals[5]
)

# Variable labels - shortened to fit on single lines
labels <- c(
  "share_bac" = "Bachelors' share",
  "share_black" = "Black pop", 
  "share_poverty" = "Poverty share",
  "logpop" = "log(Population)",
  "hpi_ch" = "House Price Growth",
  "logincome" = "log(Median Income)",
  "tightness" = "Labor Tightness",
  "pat_intensity" = "Patents/employee",
  "patai_intensity" = "AI patents' share",
  "degshare" = "Degrees/capita",
...
# Create table with shorter column headers
table_output <- modelsummary(
  list(
    "Demo." = est_demog_no,
    "Innov." = est_innovation_no,
    "Indust." = est_industry_no,
    "All" = est_all,
    "All+State" = est_all_large
  ),
  coef_map = labels,
...
**Notes:** Dependent variable is AI job share (%). All explanatory variables are standardized (z-scores). Regressions include county and year fixed effects, with state×year fixed effects in column (5). Standard errors clustered at county level. Observations are unweighted. Sources: Lightcast, American Community Survey, Quarterly Workforce Indicators, 2014-2023.
...
# Variable labels - shortened to fit on single lines
labels <- c(
  "z_share_bac" = "Bachelors",
  "z_share_black" = "Black", 
  "z_share_poverty" = "Poverty",
  "z_logpop" = "Pop. Growth",
  "z_hpi_ch" = "House Price Growth",
  "z_logincome" = "Income, Log",
  "z_tightness" = "Tightness",
  "z_pat_intensity" = "Patents/employee",
  "z_patai_intensity" = "AI Patents' Share",
  "z_degshare" = "Degrees/capita",
  "z_stemshare" = "Stem Degrees' share",
  "z_large_firms" = "Large Firms",
  "z_information_intensity" = "ICT sector Intensity",
  "z_manuf_intensity" = "Manufacturing",
  "z_TurnOvrS" = "Turnover Rate"
)
...
**Notes:** Dependent variable is AI job percentage-point share (%). All explanatory variables are standardized (z-scores). Regressions include county and year fixed effects, with state×year fixed effects in column (5). Standard errors clustered at county level. Observations are unweighted. Sources: Lightcast, American Community Survey, Quarterly Workforce Indicators, 2014-2023.
