...
<!-- DO NOT ASSUME OTHER PEOPLE ARE CORRECT! ASK AI! Constantly Am.-->

<!-- Understand what the code is doing. AI can help! Also, the code for your extensions, should be clean and understandable. For example, you only download data.csv once, even though they download it lots of times. Done -->

<!-- Rephrase this to mention 5% level: "Of the 5 statistically significant coefficients under " Fixed-->

<!-- Make clear we drop labor market tightness in our re-estimation! First sentence in the extension section. Done-->

<!-- Stop inflicting garbage upon me. I wish I were smart enough to prevent you from doing so! Done-->

<!-- Consider: "Most, but not all, of the associations persist." Is that reasonable? It is reasonable To examine, we first need the total number of significant coefficients with equal weights. Call it 15, perhaps only considering Model 4 for Tables 1 and 2. Then, of those 15, how many are still signficant? If that is 12, then sure, "Most, but not all" would be a reasonable description. But if only 7 do, then more than half are no longer significant. So, count them up and then we discuss. Done -->

<!-- Also, we need a sentence or two, maybe at the end of the Replication section, which describes this calculation, using the same sort of language as we do above. Actually, we should also have a sentence in the second to last paragraph of the introduction which covers this. Example: "Of the reported 15 significant regression coefficients in AKCLM's preferred models, only 8 remain signficant when the regressions are weighted by the log of county population." I the write up, this becomes a paragraph. "There is no single standard method for measuring how different two regression specifications are. AKCLM focus on their Model 4 for both dependendent variables: AI intensity and the change in AI intensity. With equal weighting, they estimate that the coefficients of &, *, *, ... are all statistically signficant. With log-weighted regressions, only . . .. " Done-->

<!-- Print out the paper and have a friends read it aloud to you. Or, at least, read it aloud yourself. WIP -->

<!-- All write up in terms of comparing two groups, e.g., counties with average home price compared with counties with home price growth one standard deviation higher. The second group of counties has AI job percentage with is  1.5% (?) lower. Done -->

<!-- Is there a good reason why Tables 1 and 2 differ in the width? The main reason is because there are fixed effects in table 1 which appear as longer words-->
...
models  <- list(est_demog, est_innov, est_indus, est_all, est_all_sy)
...
extra_rows <- tribble(
  ~term,           ~`(1)`, ~`(2)`, ~`(3)`, ~`(4)`, ~`(5)`,
  "Fixed effects",
    "{\\hspace{0.5em}County, Year}", "{\\hspace{0.5em}County, Year}",
    "{\\hspace{0.5em}County, Year}", "{\\hspace{0.5em}County, Year}",
    "{\\hspace{0.5em}County, state-year}",
  "Observations",
    paste0("{\\hspace{0.5em}", obs[1], "}"), paste0("{\\hspace{0.5em}", obs[2], "}"),
    paste0("{\\hspace{0.5em}", obs[3], "}"), paste0("{\\hspace{0.5em}", obs[4], "}"),
    paste0("{\\hspace{0.5em}", obs[5], "}"),
  "R²",
    paste0("{\\hspace{0.5em}", r2_vals[1], "}"), paste0("{\\hspace{0.5em}", r2_vals[2], "}"),
    paste0("{\\hspace{0.5em}", r2_vals[3], "}"), paste0("{\\hspace{0.5em}", r2_vals[4], "}"),
    paste0("{\\hspace{0.5em}", r2_vals[5], "}")
)
...
modelsummary(
  list("(1)" = est_demog, "(2)" = est_innov, "(3)" = est_indus,
       "(4)" = est_all, "(5)" = est_all_sy),
  coef_map  = labels,
  stars     = FALSE,
  gof_omit  = ".*",
  add_rows  = extra_rows,
...
models  <- list(est_t2_demog, est_t2_innov, est_t2_indus, est_t2_all, est_t2_all_state)
obs     <- map_chr(models, ~ scales::comma(nobs(.x)))
...
extra_rows <- tribble(
  ~term,           ~`(1)`, ~`(2)`, ~`(3)`, ~`(4)`, ~`(5)`,
  "Fixed effects",
    "{\\hspace{0.5em}None}", "{\\hspace{0.5em}None}",
    "{\\hspace{0.5em}None}", "{\\hspace{0.5em}None}",
    "{\\hspace{0.5em}State}",
  "Observations",
    paste0("{\\hspace{0.5em}", obs[1], "}"), paste0("{\\hspace{0.5em}", obs[2], "}"),
...
table_notes <- paste(
  "This table replicates Table 2 from AKCLM.",
  "The dependent variable is the change in AI intensity between the 2017–2018 and 2022–2023 averages,",
  "multiplied by 100 to express in percentage points.",
  "The change in AI intensity has a mean of 0.3 percentage points and a standard deviation of 1.0 percentage points.",
  "All explanatory variables are measured in 2017 and standardized as z-scores.",
  "Coefficients represent the expected difference in AI intensity growth in percentage points",
  "between two counties differing by one standard deviation in the given predictor,",
  "holding all other variables constant.",
  "Models are estimated using OLS with no fixed effects (columns 1–4) or state fixed effects (column 5).",
  "Standard errors are clustered at the county level.",
  "We reproduce all coefficients except bachelor's share, which differs systematically across",
  "all five model specifications from the published values.",
  "The authors were contacted regarding this discrepancy but did not respond."
)

modelsummary(
  list("(1)" = est_t2_demog, "(2)" = est_t2_innov, "(3)" = est_t2_indus,
       "(4)" = est_t2_all, "(5)" = est_t2_all_state),
  coef_map  = labels,
  stars     = FALSE,
  gof_omit  = ".*",
  add_rows  = extra_rows,
...
We assess robustness by re-estimating AKCLM's preferred specifications weighted by the logarithm of county population. Of the 5 coefficients that are statistically significant at the 5% level under equal weighting across both dependent variables, only 3 remain significant under logarithm-of-population weighting. For the level of AI intensity, bachelor's share and STEM degree share remain significant, while patents per employee and manufacturing intensity lose significance. For the growth of AI intensity, STEM degree share remains significant under both weighting schemes. The loss of significance for patents per employee and manufacturing intensity suggests their estimated effects on the level of AI intensity are concentrated among smaller counties.
