...
<!-- Line up columns in table. Numbers with minus signs are off by one. -->

<!-- All write up in terms of comparing two groups, e.g., counties with average home price compared with counties with home price growth one standard deviation higher. The second group of counties has AI job percentage with is  1.5% (?) lower. -->

<!-- Does the coefficient of Bachelors Share in the first regression mean 19% higher or 0.19% higher, when comparing counties that differ by one standard deviation in Bachelor's Share. -->
...
modelsummary(
  ...
  statistic = "({std.error})",
  fmt = 4,
  notes = "The table reports the demographic, innovation, and industry determinants of the share of AI jobs in 2017. Standard errors are clustered at the county level. The replication follows the exact data processing pipeline from the authors' provided code. All coefficients successfully replicate the original results except for Bachelor's share. The Bachelor's share discrepancy appears across all five model specifications, with our replicated coefficients systematically lower than the published values. The authors were contacted regarding this discrepancy but did not reply."
)
```
