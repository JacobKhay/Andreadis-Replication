...
This can bias coefficient estimates and overstate statistical significance. The issue is analogous to regressing $Y/Z$ on $Z/X$ where the shared $Z$ induces correlation even when $Y$ and $X$ are actually independent. Kronmal [-@kronmal1993spurious] shows that when the dependent variable is a ratio sharing a component with an independent variable, "regression coefficient estimates for the other independent variables... result in inappropriate conclusions." The paper recommends avoiding ratios in regression analyses when such shared components exist.
...
Table 1 presents cross-sectional estimates of the determinants of AI job intensity over 2014–2023. All regressors are standardized, permitting direct comparison of magnitudes across predictors. Column 4 — the preferred specification — includes the full control set alongside county and year fixed effects; all subsequent discussion refers to this column.
