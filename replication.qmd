...
<!-- Notes must be more extensive, at least with regard to the issue with Bachelor's share. Self-contained. -->
...
<!-- Should be possible to understand the paper by only reading the tables and figures (including their captions). Should also be possible to understand the paper without reading the table or figures. -->

<!-- So, all captions are long, about a paragraph, and include discussions of the data, especially definitions of dependent variable, years, number of counties and so on.  Yes, done.-->


<!-- Colons must be flush to journal numbers, or however they normally present things. Should not have a lot of spaces around a colon in the References. Example: Science384(6702) : 1306–1308. You have now made it (6702):1306–1308, which is better, but not correct. Should be (6702): 1306–1308. Done. -->


<!-- Should captions for all figures/tables be handled different, using fig.cap or some other trick? Investigate. Done -->
...
cat(tab)
cat("\n\n**Notes:** This table examines the demographic, innovation, and industry determinants of the share of artificial intelligence jobs across U.S. counties from 2014 to 2023. The dependent variable is AI job intensity, defined as the ratio of AI job postings to total job postings in each county-year. The sample includes approximately 3,100 U.S. counties observed annually over this ten-year period, yielding roughly 31,000 county-year observations. AI job postings are identified from Lightcast (formerly Burning Glass Technologies) online job posting data using occupation codes and skill requirements associated with artificial intelligence, machine learning, data science, and related technical competencies. All coefficients successfully replicate the original results.")
```
...
cat(tab)
cat("\n\n**Notes:** The table reports the demographic, innovation, and industry determinants of the share of AI jobs in 2017. Standard errors are clustered at the county level. The replication follows the exact data processing pipeline from the authors' provided code. All coefficients successfully replicate the original results except for Bachelor's share. The Bachelor's share discrepancy appears across all five model specifications, with our replicated coefficients systematically lower than the published values. This persistent discrepancy suggests either: (1) a difference in the underlying data file we received versus what was used in the published paper (such as a copy-paste error), (2) an undocumented data processing step in the original analysis, or (3) a potential error in the published table. The authors were contacted regarding this discrepancy but have not yet provided clarification on the source of the difference.")
```
...
The original study by @andreadis2025 employs language suggesting causal relationships despite using observational county-level data with fixed-effects regressions. The empirical strategy does not support causal inference [@holland1986], yet the paper uses terminology that implies causation rather than correlation.
