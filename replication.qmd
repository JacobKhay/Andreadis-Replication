...
<!-- DK: Do not use two different terms for our key variable, e.g., AI job concentration and AI employment.Done  -->

<!-- 9 over Official Weights -->

<!-- Exention plot legend text should be bigger. axis labels bigger. Same x-axis tick marks. Done -->

<!-- Two columns for plots. Done. -->

<!-- Extension titles/captions should be like replication title/captions, not headers.Done -->


<!-- DK: Make the font in the tables a little bigger. Font in tables can not be different sizes! Done-->
...
#| warning: false
#| message: false
#| fig-cap: "Extension of Table 1 from Andreadis et al. (2025) — Coefficients under Original weights vs Log(population) weights. This panel plot displays the coefficient estimates and 95% confidence intervals for seven key predictors across five regression models. Each panel represents a different model from the original paper. Within each panel, two estimates are shown for each variable—one using the authors' original weights and one using log(population) weights. This comparison illustrates how model weighting influences the interpretation of each predictor."
#| fig-cap-location: top
#| fig-pos: "H"
#| fig-width: 12
#| fig-height: 10
...
plots <- lapply(1:5, function(i) {
  ...
  ggplot(df, aes(x = Coef, y = Variable, color = Scheme)) +
    ...
    scale_color_manual(values = c("Original Weights" = "#1f77b4",
                       ...
                       breaks = c("Original Weights", "Log-Population Weights")) +
...
#| warning: false
#| message: false
#| fig-cap: "Extension of Table 2 from Andreadis et al. (2025) — Change in AI share under Original weights vs Log(population) weights. This figure replicates the structure of the previous figure but focuses on the change in AI employment share from 2014 to 2023. The variables selected represent core predictors of shifting AI employment. As before, each panel reflects a different model specification, with comparisons between original-weighted and log(population)-weighted regressions."
#| fig-cap-location: top
#| fig-pos: "H"
#| fig-width: 12
#| fig-height: 10
...
coefs_orig <- list(
  ...
  c(0.060, 0.087),
  c(-0.036, 0.123, 0.178),
  c(-0.136, 0.139, 0.060, 0.087, -0.036, 0.123, 0.178),
  c(-0.043, 0.168, 0.060, 0.087, -0.036, 0.123, 0.178)
)
...
make_df <- function(model_index) {
  ...
  bind_rows(
    ...
    mutate(
      ...
      upper = Coef + 1.96 * SE,
      Variable = fct_rev(factor(Variable, levels = vars)),
      Scheme = factor(Scheme, levels = c("Log-Population Weights", "Original Weights"))
    )
}
