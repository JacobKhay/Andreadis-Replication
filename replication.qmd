...
Formally, denominator contamination in this setting can be written as:

$$
AIIntensity_{it} = \frac{AIJobs_{it}}{TotalJobs_{it}}
$$

$$
LaborTightness_{it} = \frac{TotalJobs_{it}}{Unemployed_{it}}
$$

$$
\mathrm{Corr}(AIIntensity_{it}, LaborTightness_{it}) =
\mathrm{Corr}\left(\frac{AIJobs_{it}}{TotalJobs_{it}}, \frac{TotalJobs_{it}}{Unemployed_{it}}\right)
$$

The shared $TotalJobs_{it}$ term appears in both ratios, which can induce correlation mechanically, as shown in @fig-dag.

```{r}
#| label: fig-dag
#| echo: false
#| fig-cap: "DAG of denominator contamination between AI intensity and labor market tightness"
...
# Keep original DAG style while enforcing left-right order in the outputs.
...
# Display readable labels while keeping raw IDs internally.
display_labels <- c(
  AI_post = "AI Job\nPostings",
  total_post = "Total Job\nPostings",
  unemp = "Unemployed\nWorkers",
  AI_intensity = "AI Intensity",
  tightness = "Labor Market\nTightness"
)

# Draw the DAG with explicit circular bubbles.
coords <- coordinates(dag)
ed <- as.data.frame(edges(dag))
from_col <- names(ed)[1]
to_col <- names(ed)[2]

xmin <- min(coords$x) - 0.8
xmax <- max(coords$x) + 0.8
ymin <- min(coords$y) - 0.8
ymax <- max(coords$y) + 0.8
...
# Draw arrows from bubble edge to bubble edge (not center to center).
draw_edge <- function(x0, y0, x1, y1, r = node_r) {
  dx <- x1 - x0
  dy <- y1 - y0
  d <- sqrt(dx^2 + dy^2)
  if (d <= 2 * r) return(invisible(NULL))
  ux <- dx / d
  uy <- dy / d
  arrows(
    x0 = x0 + r * ux,
...
for (i in seq_len(nrow(ed))) {
  from <- as.character(ed[[from_col]][i])
  to <- as.character(ed[[to_col]][i])
  draw_edge(coords$x[from], coords$y[from], coords$x[to], coords$y[to])
}

symbols(
  x = coords$x,
  y = coords$y,
  circles = rep(node_r, length(coords$x)),
  inches = FALSE,
  add = TRUE,
  bg = "white",
  fg = "black"
...
text(
  x = coords$x,
  y = coords$y,
  labels = display_labels[names(coords$x)],
  cex = 1.08
)
```

Notes: The arrows show algebraic construction rather than causal effects. The same component, Total Job Postings, appears in both ratios, which can induce mechanical correlation.
