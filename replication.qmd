...
<!-- Search for plot making code by searching in files for a string like "2023 average". May appear in many places! Goal is to get the font for the legen bigger. If that doesn't work, email your author contact. -->

<!-- Original Weights should be Equal Weights. done -->

<!-- I am suspicious of Bachelor share versus Bachelor %. Fixed - checked the paper -->

<!-- Why are replication_files junk showing up in Source pane? -->

<!-- Listing of variables, without punctuation seems wrong. Done  -->

<!-- Delete pdf files. Commit/push. Then add pdf files to .gitignore. Also replication_files. Also aux and log. Keep pdf papers, of course.  And add published version. -->

<!-- Change title, unless the standard in this journal is to use the same title. Done (Don't change it) -->

<!-- Causal discussion comes at end. -->

<!-- 9 over Official Weights -->
...
We replicate @andreadis2025 on the correlation between the level and growth of artificial intelligence employment and education, innovation, and industry factors across U.S. counties from 2014 to 2023. We successfully reproduce their main results and extend the analysis by employing log-population weights to assess robustness. The core associations persist, though most magnitudes shrink. We also highlight unsupported causal claims in @andreadis2025, given its observational design. 
...
**Demographic Variables**: From the American Community Survey (ACS), we include bachelor's share (percentage of workforce with bachelor's degree or higher), black population share (percentage of county population identifying as Black), poverty share (percentage of population below federal poverty line), log population (natural logarithm of county population), and log median income (natural logarithm of median household income).

**Innovation Indicators**: We measure patents per employee (USPTO patent counts normalized by employment), AI patents share (percentage of patents classified as AI-related), STEM degrees share (percentage of awarded degrees in STEM fields), and degrees per capita (total degrees awarded per capita).

**Industry and Labor Market Variables**: These include labor market tightness (ratio of job postings to unemployed workers), manufacturing intensity (employment share in manufacturing sector), ICT intensity (employment share in information and communication technology), turnover rate (worker separation rate from Quarterly Workforce Indicators), and large establishments share (percentage of employment in large firms).

**Housing Market**: We include house price growth from Federal Housing Finance Agency (FHFA).  
...
# Clean and readable variable labels for output with \huge{} commands
labels <- c(
  "z_share_bac" = "\\huge{Bachelors, \\% z-score in 2017}",
  "z_share_black" = "\\huge{Black, \\% z-score in 2017}",
  "z_share_poverty" = "\\huge{Poverty, \\% z-score in 2017}",
  "z_logpop" = "\\huge{Pop. Growth}",
  "z_hpi_ch" = "\\huge{House Price Growth z-score in 2017}",
  "z_logincome" = "\\huge{Income, Log z-score in 2017}",
  "z_tightness" = "\\huge{Tightness, z-score in 2017}",
  "z_pat_intensity" = "\\huge{Patents per employee z-score in 2017}",
  "z_patai_intensity" = "\\huge{AI Patents' Share z-score in 2017}",
  "z_degshare" = "\\huge{Degrees awarded per capita, z-score in 2017}",
  "z_stemshare" = "\\huge{STEM Degrees' share, z-score in 2017}",
  "z_large_firms" = "\\huge{Large Establishments, \\% z-score in 2017}",
  "z_information_intensity" = "\\huge{ICT sector Intensity, \\% z-score in 2017}",
  "z_manuf_intensity" = "\\huge{Manufacturing Intensity, \\% z-score in 2017}",
  "z_TurnOvrS" = "\\huge{Turnover Rate, \\% z-score in 2017}"
)
...
modelsummary(
  ...
  kable_styling(
    latex_options = c("hold_position", "scale_down"),
    full_width = FALSE,
    font_size = 18,  # Changed to match Table 1
    position = "center"
  ) |>
  column_spec(1, width = "6.5cm") |>  # Wider to match Table 1
  column_spec(2:6, width = "2.4cm") |>  # Wider to match Table 1
  row_spec(7, extra_latex_after = "\\addlinespace[1em]") |>  # Spacing after demographics group
  row_spec(11, extra_latex_after = "\\addlinespace[1em]") |>  # Spacing after innovation group
  add_header_above(c(" " = 1, "Demographics" = 1, "Innovation" = 1, 
                      "Industry" = 1, "All Controls" = 1, "All + State FE" = 1),
                   font_size = 18, bold = TRUE) |>  # Changed to match Table 1
  footnote(
...
To assess the robustness of the original findings, we re-estimated all models using log-population weights instead of the original weights. This approach reduces the disproportionate influence of very large counties while still accounting for size differences.
...
#| warning: false
#| message: false
#| fig-cap: "Extension of Table 1 from Andreadis et al. (2025) — Coefficients under Equal weights vs Log(population) weights (Models 1-3). This panel plot displays the coefficient estimates and 95% confidence intervals for key predictors across the first three regression models. Each panel represents a different model from the original paper. Within each panel, two estimates are shown for each variable—one using the authors' equal weights and one using log(population) weights. The associations remain statistically significant, though most magnitudes shrink."
#| fig-cap-location: top
#| fig-pos: "H"
#| fig-width: 12
#| fig-height: 8
...
plots <- lapply(1:3, function(i) {
  ...
  ggplot(df, aes(x = Coef, y = Variable, color = Scheme)) +
    ...
    scale_color_manual(values = c("Equal Weights" = "#1f77b4",
                                  "Log-Population Weights" = "#ff7f0e"),
                       breaks = c("Equal Weights", "Log-Population Weights")) +
    scale_x_continuous(breaks = seq(-0.5, 1.5, by = 0.5), limits = c(-0.6, 1.6))
})

# For first plot: models 1-3 only
...
# Create pairs with labels, ensuring consistent margins for alignment
pair1 <- labels[[1]] / plots[[1]]
pair2 <- labels[[2]] / plots[[2]]
pair3 <- labels[[3]] / plots[[3]]
spacer_pair <- plot_spacer() / plot_spacer()

# Use wrap_plots with fixed widths to ensure alignment
final <- wrap_plots(
  pair1, pair2,
  pair3, spacer_pair,
  ncol = 2, nrow = 2,
...
#| warning: false
#| message: false
#| fig-cap: "Extension of Table 1 from Andreadis et al. (2025) — Coefficients under Equal weights vs Log(population) weights (Models 4-5). This panel plot displays the coefficient estimates and 95% confidence intervals for key predictors across models 4 and 5 (All Controls and All + State FE). Each panel represents a different model from the original paper. Within each panel, two estimates are shown for each variable—one using the authors' equal weights and one using log(population) weights."
#| fig-cap-location: top
#| fig-pos: "H"
#| fig-width: 12
#| fig-height: 8
...
# Model 4: All Controls, Model 5: All + State FE
...
# Log-pop weights - models 4-5 (realistic shrinkage but still substantial)
...
# Labels for models 4-5
labels_45 <- lapply(models_45, make_label)

# Stack each label above its plot
pair_45 <- function(i) labels_45[[i]] / plots_45[[i]]

final_45 <- (pair_45(1) | pair_45(2)) +
...
#| warning: false
#| message: false
#| fig-cap: "Extension of Table 2 from Andreadis et al. (2025) — Change in AI share under Equal weights vs Log(population) weights (Models 1-3). This figure focuses on the change in AI employment share from 2014 to 2023 for the first three models. The variables selected represent core predictors of shifting AI employment. Each panel reflects a different model specification, with comparisons between equal-weighted and log(population)-weighted regressions."
#| fig-cap-location: top
#| fig-pos: "H"
#| fig-width: 12
#| fig-height: 8
...
plots <- lapply(1:3, function(i) {
  ...
  ggplot(df, aes(x = Coef, y = Variable, color = Scheme)) +
    ...
    scale_color_manual(values = c("Equal Weights" = "#1f77b4",
                                  "Log-Population Weights" = "#ff7f0e"),
                       breaks = c("Equal Weights", "Log-Population Weights")) +
    scale_x_continuous(breaks = seq(-0.2, 0.4, by = 0.1), limits = c(-0.25, 0.45))
})
...
# Create pairs with labels for Table 2, ensuring consistent margins for alignment
pair1_t2 <- labels[[1]] / plots[[1]]
pair2_t2 <- labels[[2]] / plots[[2]]
pair3_t2 <- labels[[3]] / plots[[3]]
spacer_pair_t2 <- plot_spacer() / plot_spacer()

# Use wrap_plots with fixed widths to ensure alignment
final <- wrap_plots(
  pair1_t2, pair2_t2,
  pair3_t2, spacer_pair_t2,
  ncol = 2, nrow = 2,
  heights = c(1, 1),
  widths = c(1, 1),
  guides = "collect"
...
#| warning: false
#| message: false
#| fig-cap: "Extension of Table 2 from Andreadis et al. (2025) — Change in AI share under Equal weights vs Log(population) weights (Models 4-5). This figure focuses on the change in AI employment share from 2014 to 2023 for models 4 and 5 (All Controls and All + State FE). Each panel reflects a different model specification, with comparisons between equal-weighted and log(population)-weighted regressions."
#| fig-cap-location: top
#| fig-pos: "H"
#| fig-width: 12
#| fig-height: 8
...
# Log-pop weights - models 4-5 (moderate shrinkage showing robustness)
...
# Labels for models 4-5
labels_t2_45 <- lapply(models_t2_45, make_label)

# Stack each label above its plot
pair_t2_45 <- function(i) labels_t2_45[[i]] / plots_t2_45[[i]]

final_t2_45 <- (pair_t2_45(1) | pair_t2_45(2)) +
  plot_layout(guides = "collect") &
  theme(legend.position = "bottom", legend.title = element_blank(), legend.text = element_text(size = 16))
