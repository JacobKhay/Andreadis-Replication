...
We partially replicate @andreadis2025 on the correlation between the level and growth of artificial intelligence employment across U.S. counties from 2014 to 2023 and a collection of variables involving education, innovation, and industry. We successfully reproduce most of their results and extend their analysis by using log-population weights to assess robustness. Only some of the associations persist. We also critique their variable construction and unsupported causal claims.
...
This creates denominator contamination or mechanical endogeneity, where both sides of the regression share the same underlying quantity (total job postings), inducing a built-in correlation that exists purely through algebraic construction. This can bias coefficient estimates and overstate statistical significance. The issue is analogous to regressing $Y/Z$ on $Z/X$ where the shared $Z$ induces correlation even when $Y$ and $X$ are actually independent. 

<!-- DK: Cite a paper, ideally one which includes a sentence saying clearly not to do this. Ideally, the sentence will mention that including this bad variable biases the other coefficients, which it does!  -->
...
variables_list <- list(
  c("Bachelor's share", "House price growth", "Labor Tightness"),
...
coefs_orig_list <- list(
  c(coef(est_demog_no)["share_bac"], coef(est_demog_no)["hpi_ch"], coef(est_demog_no)["tightness"]),
...
ses_orig_list <- list(
  c(se(est_demog_no)["share_bac"], se(est_demog_no)["hpi_ch"], se(est_demog_no)["tightness"]),
...
coefs_log_list <- list(
  c(coef(est_demog_logpop)["share_bac"], coef(est_demog_logpop)["hpi_ch"], coef(est_demog_logpop)["tightness"]),
...
ses_log_list <- list(
  c(se(est_demog_logpop)["share_bac"], se(est_demog_logpop)["hpi_ch"], se(est_demog_logpop)["tightness"]),
...
make_df <- function(model_index) {
  ...
  bind_rows(
    ...
    mutate(
      ...
      Scheme = factor(Scheme, levels = c("Equal Weights", "Log-Population Weights"))
    )
}
...
plots <- lapply(1:3, function(i) {
  ...
  ggplot(df, aes(x = Coef, y = Variable, color = Scheme)) +
    ...
    scale_color_manual(
      values = c("Equal Weights" = "#F24236", "Log-Population Weights" = "#28A745"),
...
variables_45 <- c(
  "Bachelor's share", "House price growth", "Labor Tightness",
...
coefs_orig_list_45 <- list(
  ...
  c(coef(est_all_state_year)["share_bac"], coef(est_all_state_year)["hpi_ch"],
    coef(est_all_state_year)["tightness"], coef(est_all_state_year)["pat_intensity"],
...
ses_orig_list_45 <- list(
  ...
  c(se(est_all_state_year)["share_bac"], se(est_all_state_year)["hpi_ch"],
    se(est_all_state_year)["tightness"], se(est_all_state_year)["pat_intensity"],
...
coefs_log_list_45 <- list(
  c(coef(est_all_logpop)["share_bac"], coef(est_all_logpop)["hpi_ch"],
    coef(est_all_logpop)["tightness"], coef(est_all_logpop)["pat_intensity"],
  ...
  c(coef(est_all_state_year_logpop)["share_bac"], coef(est_all_state_year_logpop)["hpi_ch"],
    coef(est_all_state_year_logpop)["tightness"], coef(est_all_state_year_logpop)["pat_intensity"],
...
ses_log_list_45 <- list(
  c(se(est_all_logpop)["share_bac"], se(est_all_logpop)["hpi_ch"],
    se(est_all_logpop)["tightness"], se(est_all_logpop)["pat_intensity"],
  ...
  c(se(est_all_state_year_logpop)["share_bac"], se(est_all_state_year_logpop)["hpi_ch"],
    se(est_all_state_year_logpop)["tightness"], se(est_all_state_year_logpop)["pat_intensity"],
...
variables_list <- list(
  c("Bachelors Share", "House Price Growth", "Tightness"),
...
coefs_actual_list <- list(
  c(coef(est_t2_demog_no)["share_bac14"], coef(est_t2_demog_no)["hpi_ch14"], coef(est_t2_demog_no)["tightness14"]),
...
ses_actual_list <- list(
  c(se(est_t2_demog_no)["share_bac14"], se(est_t2_demog_no)["hpi_ch14"], se(est_t2_demog_no)["tightness14"]),
...
coefs_logpop_list <- list(
  c(coef(est_t2_demog_logpop)["share_bac14"], coef(est_t2_demog_logpop)["hpi_ch14"], coef(est_t2_demog_logpop)["tightness14"]),
...
ses_logpop_list <- list(
  c(se(est_t2_demog_logpop)["share_bac14"], se(est_t2_demog_logpop)["hpi_ch14"], se(est_t2_demog_logpop)["tightness14"]),
...
variables_list_t2_45 <- list(
  c("Bachelors Share", "House Price Growth", "Tightness", "Patents per emp.", "STEM Degrees' share",
